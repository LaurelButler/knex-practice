--BEGIN and COMMIT are like a try block in JavaScript
--postgresql's CREATE or INSERT are transactions 
--adding BEGIN before each command and COMMIT after each are specifying the start and end of a transaction; this combines them into one transaction
